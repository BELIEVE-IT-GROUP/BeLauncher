...
struct SearchPerformanceTests {
    ...
    /// run remains the strict signal; the full suite gets an explicit contention allowance because
    /// Swift Testing runs unrelated SQLite and graph work at the same time on the release runner.
    ...
    func keystrokeStaysUnderAFrame() {
        ...
        #expect(milliseconds < 90, "a keystroke took \(Int(milliseconds)) ms — investigate the launcher path")
    }
